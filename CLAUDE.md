...
is one edit. `game/extracted/` holds the two tables a build reads back out of
the `.ain` and commits: which string slots the enemy hints are, and which number
is which race. Those are regenerated by their own script, never edited — the
English for both lives in `glossaries/`.
